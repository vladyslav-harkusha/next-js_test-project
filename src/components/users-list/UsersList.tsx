...
export default async function UsersList({ searchParams }: Props) {
    ...
    const usersData: IUsersResponse | undefined = await getEntitiesByUrlParams<IUsersResponse>({
    ...
    return (
        <div className='users-list'>
            <p className='users-list-description'>
                {usersData.users.length ? 'click on user item to see user details' : 'no users found'}
            </p>
            <ul>
